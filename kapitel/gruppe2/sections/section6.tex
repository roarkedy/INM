...
Das Shibboleth-Verfahren (Verfahren zur Verteilten Authentifizierung und Autorisierung für Webanwendungen) kommt an der Hochschule zum Einsatz. Durch dieses System können Dienste bestimmter Anbieter mit dem Hochschullogin genutzt werden, ohne das dem Anbieter der Nutzername und Passwort mitgeteilt wird (siehe Abbildung \ref{fig_shibboleth_hs}).\footnote{\url{http://www.hs-emden-leer.de/einrichtungen/rechenzentrum/infos-downloads-und-anleitungen/shibboleth.html}} Es wird also ein zentralisierter Account für die Authentifizierung genutzt. 

\begin{figure}
	\centering
	\includegraphics[width=8cm]{kapitel/gruppe2/bilder/shibboleth}
	\caption{Shibboleth-Login der Hochschule}
	\label{fig_shibboleth_hs}
\end{figure}

Das Shibboleth-Verfahren ermöglicht den Studierenden und Mitarbeitern die Nutzung folgender Dienste:
...
Hierbei muss bei der Anmeldung die Hochschule als Heimatinstitution und auf der Login-Seite die Hochschulkennung (Benutzername + Kennwort)angegeben werden.\footnote{\url{http://www.hs-emden-leer.de/no_cache/einrichtungen/bibliothek/medienangebot/elektronische-angebote/vpn-shibboleth.html}}

Single-Sign-On (siehe Kapitel \ref{realisierung_der_serviceorientierung} auf Seite \pageref{realisierung_der_serviceorientierung})) wurde an der Hochschule noch nicht für alle Systeme umgesetzt. Für den Zugriff auf die Datenlaufwerke (siehe Kapitel \ref{zentrale_Datenlaufwerke} auf Seite \pageref{zentrale_Datenlaufwerke}) kommt Single-Sign-On zum Einsatz, da nach der Anmeldung am Client keine weiteren Benutzerdaten zur Authentifizierung eingegeben werden müssen. 
